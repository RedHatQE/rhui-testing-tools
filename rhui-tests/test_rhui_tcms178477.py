...
class test_tcms_178477(RHUITestcase, RHUI_has_RH_cert, RHUI_has_RHEL5_and_RHEL6_CLIs):
    def _setup(self):
        ...
        RHUIManagerRepo.add_rh_repo_by_repo(self.rs.Instances["RHUA"][0], ["Red Hat Enterprise Linux 6 Server from RHUI \(RPMs\) \(6Server-x86_64\)", "Red Hat Enterprise Linux 5 Server from RHUI \(RPMs\) \(5Server-x86_64\)"])

        '''[TCMS#178477 setup] Associate repo with clusters '''
        RHUIManagerCds.associate_repo_cds(self.rs.Instances["RHUA"][0], "Cluster1", ["Red Hat Enterprise Linux 6 Server from RHUI \(RPMs\) \(6Server-x86_64\)", "Red Hat Enterprise Linux 5 Server from RHUI \(RPMs\) \(5Server-x86_64\)"])
        ...
        RHUIManagerClient.generate_ent_cert(self.rs.Instances["RHUA"][0], "Cluster1", ["Red Hat Enterprise Linux 5 Server from RHUI \(RPMs\)"], "cert1", "/root/", validity_days="", cert_pw=None)
        RHUIManagerClient.generate_ent_cert(self.rs.Instances["RHUA"][0], "Cluster1", ["Red Hat Enterprise Linux 6 Server from RHUI \(RPMs\)"], "cert2", "/root/", validity_days="", cert_pw=None)

        '''[TCMS#178477 setup] Create configuration rpms '''
        RHUIManagerClient.create_conf_rpm(self.rs.Instances["RHUA"][0], "Cluster1", self.rs.Instances["CDS"][0].private_hostname, "/root", "/root/cert1.crt", "/root/cert1.key", "repo1", "3.0")
        RHUIManagerClient.create_conf_rpm(self.rs.Instances["RHUA"][0], "Cluster1", self.rs.Instances["CDS"][0].private_hostname, "/root", "/root/cert2.crt", "/root/cert2.key", "repo2", "3.0")

        '''[TCMS#178477 setup] Install configuration rpms to the clients '''
        Util.install_rpm_from_rhua(self.rs.Instances["RHUA"][0], self.rhel5client, "/root/repo1-3.0/build/RPMS/noarch/repo1-3.0-1.noarch.rpm")
        Util.install_rpm_from_rhua(self.rs.Instances["RHUA"][0], self.rhel6client, "/root/repo2-3.0/build/RPMS/noarch/repo2-3.0-1.noarch.rpm")
    ...
    def _cleanup(self):
        ...
        RHUIManagerRepo.delete_repo(self.rs.Instances["RHUA"][0], ["Red Hat Enterprise Linux 6 Server from RHUI \(RPMs\) \(6Server-x86_64\)", "Red Hat Enterprise Linux 5 Server from RHUI \(RPMs\) \(5Server-x86_64\)"])
