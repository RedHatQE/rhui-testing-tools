...
class test_tcms_110761(RHUITestcase, RHUI_has_two_CDSes):
    ...
    def _test(self):
        ...
        Expect.ping_pong(self.rs.CLI[0], "sed -i '/^" + self.rs.CDS[0].private_hostname + "$/d' /etc/yum.repos.d/rhui-load-balancers && echo SUCCESS", "[^ ]SUCCESS", 60)
        Expect.ping_pong(self.rs.CLI[0], "yum clean all && echo SUCCESS", "[^ ]SUCCESS", 60)
        Expect.ping_pong(self.rs.CLI[0], "yum install -y custom-signed-rpm && echo SUCCESS", "[^ ]SUCCESS", 60)
        Expect.ping_pong(self.rs.CLI[0], "rpm -e custom-signed-rpm && echo SUCCESS", "[^ ]SUCCESS", 60)
