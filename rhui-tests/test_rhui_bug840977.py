...
class test_bug_840977(RHUITestcase):
    ...
    def _test(self):
        ...
        Expect.enter(self.rs.Instances["RHUA"][0], "i")
        RHUIManager.select(self.rs.Instances["RHUA"][0], ["Cluster1"])
        Expect.expect(self.rs.Instances["RHUA"][0], "Repositories.*\(None\).*rhui \(cds\) =>")
