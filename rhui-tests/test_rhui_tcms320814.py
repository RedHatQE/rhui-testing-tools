...
class test_tcms_320814(RHUITestcase, RHUI_has_RH_cert):
    def _setup(self):
        '''[TCMS#320814 setup] Do initial rhui-manager run'''
        RHUIManager.initial_run(self.rs.Instances["RHUA"][0])

        '''[TCMS#320814 setup] Upload RH content certificate '''
        RHUIManagerEntitlements.upload_content_cert(self.rs.Instances["RHUA"][0], self.cert)

        '''[TCMS#320814 setup] Add rh repo '''
        ...
        '''[TCMS#320814 setup] Sync rh repo '''
    ...
    def _test(self):
        '''[TCMS320814 test] Wait untill repo is synced '''
        ...
        '''[TCMS#320814 test] Check the status of the repo'''
        Expect.ping_pong(self.rs.Instances["RHUA"][0], "rhui-manager status | grep \"Red Hat Enterprise Linux 5 Server Beta from RHUI (Debug RPMs) (5Server-x86_64)\" | grep \"SUCCESS\" && echo SUCCESS", "[^ ]SUCCESS", 20)
