...
class test_0_update_simple(RHUITestcase, RHUI_has_RH_cert):
    def _setup(self):
        ...
        Util.remove_conf_rpm(self.rs.RHUA[0])
        ...
        Expect.ping_pong(self.rs.RHUA[0], "yum install -y /root/repo1-3.0/build/RPMS/noarch/repo1-3.0-1.noarch.rpm && echo SUCCESS", "[^ ]SUCCESS", 60)

    def _test(self):
        '''[Update Simple test] Upgrade RHUI '''
        Expect.ping_pong(self.rs.RHUA[0], "yum -y update && echo SUCCESS", "[^ ]SUCCESS", 120)
