...
class test_tcms_90683(RHUITestcase):
    ...
    def _test(self):
        ...
        Expect.ping_pong(self.rs.CDS[0], "service pulp-cds stop && echo SUCCESS", "[^ ]SUCCESS", 30)

        '''[TCMS#90683 test] Change gopher password location '''
        Expect.ping_pong(self.rs.CDS[0], "sed -i 's,^secret_file = .*$,secret_file = /var/lib/pulp-cds/.gofer/newsecret,' /etc/gofer/plugins/cdsplugin.conf && echo SUCCESS", "[^ ]SUCCESS")

        '''[TCMS#90683 test] Start pulp-cds '''
        Expect.ping_pong(self.rs.CDS[0], "service pulp-cds start && echo SUCCESS", "[^ ]SUCCESS", 30)
        ...
        Expect.ping_pong(self.rs.CDS[0], "[ -f /var/lib/pulp-cds/.gofer/newsecret ] && echo SUCCESS", "[^ ]SUCCESS", 30)
    ...
    def _cleanup(self):
        ...
        Expect.ping_pong(self.rs.CDS[0], "sed -i 's,^secret_file = .*$,secret_file = /var/lib/pulp-cds/.gofer/secret,' /etc/gofer/plugins/cdsplugin.conf && echo SUCCESS", "[^ ]SUCCESS")
