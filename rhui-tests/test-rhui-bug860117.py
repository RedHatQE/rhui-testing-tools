...
class test_bug_860117(RHUITestcase, RHUI_has_two_CDSes):
    def _setup(self):
        ...
        RHUIManagerRepo.add_custom_repo(self.rs.RHUA[0], "repo4")
        RHUIManagerRepo.add_custom_repo(self.rs.RHUA[0], "repo5")
        RHUIManagerRepo.add_custom_repo(self.rs.RHUA[0], "repo6")

    def _test(self):
        '''[Bug#860117 test] Associate custom repos with cluster '''
        RHUIManagerCds.associate_repo_cds(self.rs.RHUA[0], "Cluster1", ["repo1", "repo3", "repo6"])

        '''[Bug#860117 test] Generate entitlement certificate '''
        RHUIManagerClient.generate_ent_cert(self.rs.RHUA[0], "Cluster1", ["repo1", "repo3", "repo6"], "cert-repo1", "/root/", validity_days="", cert_pw=None)
    ...
    def _cleanup(self):
        ...
        RHUIManagerRepo.delete_repo(self.rs.RHUA[0], ["repo1", "repo2", "repo3", "repo4", "repo5", "repo6"])
