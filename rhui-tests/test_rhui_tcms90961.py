...
class test_tcms_90961(RHUITestcase, RHUI_has_RH_cert):
    ...
    def _test(self):
        self.rs.Instances["RHUA"][0].sftp.put(self.cert, "/root/test_90961.pem")

        '''[TCMS#90961 test] Test client with second cds only '''
        Expect.ping_pong(self.rs.Instances["RHUA"][0], "rhui-manager cert upload --cert /root/test_90961.pem | grep 'Red Hat Update Infrastructure 2' && echo SUCCESS", "[^ ]SUCCESS", 10)
