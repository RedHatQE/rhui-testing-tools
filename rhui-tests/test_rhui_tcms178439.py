...
class test_tcms_178439(RHUITestcase):
    ...
    def _setup(self):
        ...
        RHUIManagerCds.add_cds(self.rs.RHUA[0], self.cluster_a,
                self.rs.CDS[0].private_hostname)

    def _test(self):
        """[TCMS#178439 test] move the cds to cluster_b"""
        RHUIManagerCds.move_cds(self.rs.RHUA[0], [self.rs.CDS[0].private_hostname],
                self.cluster_b)
        ...
        rhui_cdses = RHUIManagerCds.info(self.rs.RHUA[0], [self.cluster_b])
    ...
    def _cleanup(self):
        ...
        RHUIManagerCds.delete_cds(self.rs.RHUA[0], self.cluster_b,
                [self.rs.CDS[0].private_hostname])
