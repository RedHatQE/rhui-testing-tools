...
class test_tcms_178473(RHUITestcase, RHUI_has_two_CDSes):
    def _setup(self):
        ...
        RHUIManagerCds.add_cds(self.rs.RHUA[0], "Cluster2", self.rs.CDS[1].private_hostname, self.rs.CDS[1].public_hostname)
    ...
    def _test(self):
        ...
        cds0 = RhuiCds(
                ...
                client_hostname=self.rs.CDS[0].public_hostname,
                cluster="Cluster1",
                repos=["repo1"]
                )
        cds1 = RhuiCds(
                hostname=self.rs.CDS[1].private_hostname,
                client_hostname=self.rs.CDS[1].public_hostname,
                cluster="Cluster2",
                repos=["repo1"]
                )
