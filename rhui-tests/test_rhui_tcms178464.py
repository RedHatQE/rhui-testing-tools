...
class test_tcms_178464(RHUITestcase):
    ...
    def _test(self):
        ...
        nose.tools.assert_equal(RHUIManagerCds.info(self.rs.RHUA[0], ["Cluster1"]), [cds])

        '''[TCMS#178464 test] Check pulp-admin cds list '''
        nose.tools.assert_equals(PulpAdmin.cds_list(self.rs.RHUA[0]),
                RHUIManagerCds.info(self.rs.RHUA[0], ["Cluster1"]))

        '''[TCMS#178464 test] Check certs created for custom repo '''
        Expect.ping_pong(self.rs.RHUA[0], "test -f /etc/pki/pulp/content/repo1/consumer-repo1.cert && echo SUCCESS", "[^ ]SUCCESS")
        Expect.ping_pong(self.rs.RHUA[0], "test -f /etc/pki/pulp/content/repo1/consumer-repo1.ca && echo SUCCESS", "[^ ]SUCCESS")
