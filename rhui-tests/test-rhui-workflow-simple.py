...
class test_workflow_simple(RHUITestcase, RHUI_has_RH_cert):
    ...
    def _test(self):
        ...
        for cds in self.rs.CDS:
            RHUIManagerCds.add_cds(self.rs.RHUA[0], "Cluster1", cds.private_hostname)
        ...
        RHUIManagerCds.associate_repo_cds(self.rs.RHUA[0], "Cluster1", ["repo1", "repo2"])

        '''[Simple Workflow test] Upload content to custom repo '''
        RHUIManagerRepo.upload_content(self.rs.RHUA[0], ["repo1", "repo2"], "/etc/rhui/confrpm")
        ...
        ## RHUIManagerRepo.add_rh_repo_by_product(self.rs.RHUA[0], ["Red Hat Enterprise Linux 6 Server - Supplementary from RHUI \(RPMs\)", "Red Hat Enterprise Linux 6 Server from RHUI \(RPMs\)"])
        ...
        ## RHUIManagerRepo.add_rh_repo_all(self.rs.RHUA[0])
        ...
        RHUIManagerClient.generate_ent_cert(self.rs.RHUA[0], "Cluster1", ["repo1", "Red Hat Update Infrastructure 2 \(RPMs\)"], "cert-repo1", "/root/", validity_days="", cert_pw=None)
        ...
        RHUIManagerIdentity.generate_new(self.rs.RHUA[0])

        '''[Simple Workflow test] Change password '''
        RHUIManagerUsers.change_password(self.rs.RHUA[0], "admin", "admin2")
        RHUIManagerUsers.change_password(self.rs.RHUA[0], "admin", "admin")

    def _cleanup(self):
        '''[Simple Workflow cleanup] Remove cdses '''
        for cds in self.rs.CDS:
            RHUIManagerCds.delete_cds(self.rs.RHUA[0], "Cluster1", [cds.private_hostname])

        '''[Simple Workflow cleanup] Delete custom repos '''
        RHUIManagerRepo.delete_repo(self.rs.RHUA[0], ["repo1", "repo2", "Red Hat Update Infrastructure 2 \(RPMs\) \(6Server-x86_64\)"])
