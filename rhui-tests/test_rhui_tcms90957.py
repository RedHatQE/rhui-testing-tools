...
class test_tcms_90957(RHUITestcase, RHUI_has_two_CDSes):
    ...
    def _test(self):
        ...
        Expect.ping_pong(self.rs.Instances["RHUA"][0], "rhui-manager status | grep '" + self.rs.Instances["CDS"][0].private_hostname + " ' | grep 'DOWN' && echo SUCCESS", "[^ ]SUCCESS", 10)
        Expect.ping_pong(self.rs.Instances["RHUA"][0], "rhui-manager status | grep '" + self.rs.Instances["CDS"][1].private_hostname + " ' | grep 'UP' && echo SUCCESS", "[^ ]SUCCESS", 10)
