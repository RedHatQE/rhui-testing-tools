...
class test_tcms_90968(RHUITestcase, RHUI_has_RH_cert):
    ...
    def _test(self):
        ...
        Expect.ping_pong(self.rs.RHUA[0], "rhui-manager repo info --repo_id repo1 | grep '^Type:[[:space:]]*Custom' && echo SUCCESS", "[^ ]SUCCESS", 10)
        Expect.ping_pong(self.rs.RHUA[0], "rhui-manager repo info --repo_id rhel-x86_64-6-rhui-2-rpms-6Server-x86_64 | grep '^Type:[[:space:]]*Red Hat' && echo SUCCESS", "[^ ]SUCCESS", 10)
